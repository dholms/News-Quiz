    ...
    $.quiz = function(quiz_data, options) {
      ...
      container : 'quiz_container',
            ...
            append_how_you_did_section: function() {
                ...
                personal_message = jQuery('<span>You didn\'t get any right!</span> <br/>');
                var how_you_did_element = jQuery('<p class="how_you_did"></p>');
                how_you_did_element.append('<span>');
                how_you_did_element.append(personal_message);
                how_you_did_element.append('</span> </br>');
      ...
      calculate_aspectratios: function(data) {
        ...
        if (   (self.defaulting_behavior_on && row[correct + wrong_number + value] !== self.defaulting_flag)        
          ...
          if (self.possible_display_elements[i - 1].finder(container).length ) {
            ...
            update_personal_message: function(){
                if(right_answers < 2){
                  personal_message.text('Did you even pay attention?');
                }
                else if (right_answers < 4){
                  personal_message.text('Nice Work!');
                }  
                else{
                  personal_message.text('Wow! You did Great'); }
            }
        };
